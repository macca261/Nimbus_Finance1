...
export function DonutArea({ uploadId, categories, info }: { uploadId: number; categories: Array<{ key: string; total: number }>; info?: string }) {
  ...
  React.useEffect(() => { if (!selected && categories[0]) setSelected(categories[0].key); }, [categories, selected]);
        ...
        {empty ? (
          <div style={{ padding:16, border:'1px solid #E6E9F2', borderRadius:12, background:'#fff' }}>
            ...
            <div style={{ color:'#6B7280', marginTop:6 }}>
              {info === 'no uploads yet' ? 'Bitte CSV hochladen.' : 'Lade Kategorien… oder keine Kategorien für diesen Upload.'}
            </div>
